...
h5Seurat_file = here('data/tidy_data/rdas/final_cluster_assignment_countsplit.h5seurat')
...
if(!file.exists(h5Seurat_file)){
  ...
  Xtest <- split$test %>% as("sparseMatrix")
  rm(split); gc()
  ...
  Xtestsubset <- Xtest[rownames(obj.train),colnames(obj.train)] ## create subset for later
  dim(Xtestsubset)

  ## add the test data from count split
  obj.train[['countsplit']] <- CreateAssayObject(counts=Xtestsubset)
  obj.train <- obj.train %>% NormalizeData(assay="countsplit") %>% 
    ScaleData(assay="countsplit")
  
  ## save this version
  SaveH5Seurat(obj.train, h5Seurat_file, overwrite = T)
}
...
h5Seurat_file2 = here('data/tidy_data/rdas/Russ_et_al_dorsal_horn_cells_countsplit.h5seurat')
...
h5Seurat_file3 = here('data/tidy_data/rdas/Russ_et_al_dorsal_horn_neurons_countsplit.h5seurat')
...
## smooth out the genes with imputation across full data
DefaultAssay(obj_dhn) <- "countsplit"
obj_dhn <- magic(obj_dhn, genes='all_genes')
